...
const getAllParagraphs = (pool) => (story) => {
  const paragraphsQuery = `SELECT paragraphs.id, paragraphs.created_user_id, users.realname AS created_username, paragraphs.last_updated_user_id, paragraphs.story_id, paragraphs.paragraph FROM paragraphs INNER JOIN users ON users.id=paragraphs.created_user_id WHERE story_id=${story.id} ORDER BY id ASC`;
...
const getOrSetStoryCollab = (pool, request) => new Promise((resolve, reject) => {
  // think of a way to ensure that each function can be modularised further,
  // so that they can be reused instead of being stuck in this function
